...
export function RatingRow({ 
  ...
  return (
    <div className="group flex flex-col sm:flex-row sm:items-center justify-between rounded-none px-4 py-4 transition-colors hover:bg-orange-500/5 border-b border-stone-100 last:border-0 gap-4">
      <span className="truncate text-[14px] font-medium text-stone-800">{label}</span>
      <div className="flex items-center gap-6 flex-1 sm:max-w-[240px]">
        <Slider
          value={[value]}
        ...
        />
        <div className={cn(
          "w-10 h-10 flex items-center justify-center rounded-lg text-sm font-bold shadow-sm transition-transform group-hover:scale-110 shrink-0",
          value >= 16 ? "bg-orange-600 text-white" :
          value >= 13 ? "bg-orange-400 text-white" :
          value >= 10 ? "bg-amber-100 text-amber-800" :
